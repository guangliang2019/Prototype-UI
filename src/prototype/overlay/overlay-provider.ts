  ...
  implements OverlayProviderProps
{
  ...
  connectedCallback(): void {
    ...
    // overlay oepn 事件通过冒泡被 proivder 劫持，然后关闭 overlayKey 相同的其他 overlay
    this.addEventListener('overlay-oepn', this._handleOverlayOpen as EventListener);
  }
  // 同 overlayKey 的 overlay 同时只能存在一个，其他会在另一个调用 oepn 的同时进行 close
  // TODO: 让 overlay 在 oepn 的时候事件中带上一个 close 时会销毁的引用，然后通过 weakmap 来维护，比遍历性能更好
  private _handleOverlayOpen(event: CustomEvent<OpenOverlayEventDetail>) {
    ...
    for (const consumer of consumers) {
      if (
        // 必须是开着的
        consumer.getAttribute('data-oepn') !== null &&
        // overlayKey 相同
